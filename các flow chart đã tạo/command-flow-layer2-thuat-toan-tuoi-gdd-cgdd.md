...
Chỉ các biến dưới đây mới được tính là biến Layer 2 tạo ra.

### 1. Biến tạo ra để gửi xuống Layer 3
...
```text
pump_cmd: lệnh bật hoặc tắt bơm.
relay: trạng thái relay cần điều khiển theo lệnh bơm.
pump_state: trạng thái bơm mong muốn.
WATER_DURATION_MS: thời gian tưới sau khi xét Stage, H_soil, T_air, H_air và MIN_WATER_INTERVAL.
control_status: trạng thái quyết định điều khiển.
timestamp: thời điểm tạo gói ControlData_t.
ControlData_t: gói dữ liệu điều khiển gửi từ Layer 2 xuống Layer 3.
```

### 2. Biến Layer 2 tạo ra hoặc cập nhật nội bộ
...
```text
GDD: giá trị GDD trong ngày.
CGDD: tổng GDD tích lũy.
current_stage: giai đoạn cây hiện tại.
last_watering_time: thời điểm tưới gần nhất để kiểm tra MIN_WATER_INTERVAL.
```
